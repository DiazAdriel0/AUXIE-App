        ...
        <div className='flex'>
            ...
            <div className='grid grid-cols-2'>
                        ...
                        <div className='grid grid-cols-2 grid-rows-2 gap-x-80 gap-y-0.5 px-4'>
                            {services ? (
                                services.map(service => (
                                    <div className="flex flex-col items-center justify-center h-20 w-44 bg-white border rounded-lg p-4 space-y-2" key={service.name}>
                                        <img className="w-full h-full rounded-t-lg" src={service.image?.secure_url} alt={service.name} />
                ...
                {payment?.map((service, index) => (
                                <tr key={index}>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{service.service}</td>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{service.price}</td>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{service.paymentMethod}</td>
                                </tr>
                            ))}
                </tbody>
                    </table>
                    ...
                    <table className='mx-auto border-collapse mt-2 ml-20'>
                        ...
                        <tbody>
                            {lastJobs?.map((review, index) => (
                                <tr key={index}>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100  '>{review.service}</td>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{review.review}</td>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{review.score}</td>
                                    <td className='py-2 px-4 border border-gray-300 bg-gray-100 '>{review.username}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
